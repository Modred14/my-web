...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      // If scrolled past the reviews section, clear active highlight
      const reviewsEl = document.getElementById("reviews");
      if (reviewsEl) {
        const reviewsBottom = reviewsEl.getBoundingClientRect().bottom + window.scrollY;
        if (window.scrollY + window.innerHeight >= reviewsBottom + 80) {
          setActiveSection("");
          return;
        }
      }

      // Walk sections top-down; last one whose top <= triggerY wins
      let current = "about";
      for (const id of ids) {
    ...
    window.addEventListener("scroll", onScroll, { passive: true });
    onScroll(); // run once on mount so About is immediately active
    return () => window.removeEventListener("scroll", onScroll);
  }, []);
  ...
  return (
    <>
      ...
      <div
        onClick={close}
        className={`fixed inset-0 z-[200] h-screen bg-black/60  transition-opacity duration-300 md:hidden ${
      ...
      {/* Drawer */}
      <aside
        className={`fixed top-0 right-0 z-[201] h-screen w-[280px] bg-[#070d1a] border-l border-white/[0.08]
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-xs tracking-widest uppercase text-white/80">Modred.dev</span>
          </div>
          <button
            onClick={close}
        ...
        <nav className="flex flex-col gap-1 px-4 pt-6 flex-1">
          {navLinks.map((link, i) => {
            ...
            return (
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isActive ? "bg-blue-400" : "bg-white/10"}`} />
                {link.label}
              </Link>
            );
          })}
        ...
        <div className="px-4 py-6 border-t border-white/[0.06] flex flex-col gap-3">
          ...
          <p className="text-center font-mono text-xs text-gray-600 tabular-nums">{currentTime}</p>
        </div>
      </aside>
    </>
  );
